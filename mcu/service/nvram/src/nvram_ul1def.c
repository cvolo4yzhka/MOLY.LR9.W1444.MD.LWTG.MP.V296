...
void nvram_init_uL1_tasAntIdxByBand(void)
{
   kal_bool error_no;

   UMTS_TAS_ANT_IDX_T *ul1_tas_ant_idx_buffer = (UMTS_TAS_ANT_IDX_T *)get_ctrl_buffer(sizeof(UMTS_TAS_ANT_IDX_T));

   error_no = nvram_external_read_data(NVRAM_EF_UL1_TAS_ANT_IDX_TEST_SIM_LID, 1, (kal_uint8 *)ul1_tas_ant_idx_buffer, sizeof(UMTS_TAS_ANT_IDX_T));
   ASSERT(error_no == KAL_TRUE);

   kal_mem_cpy(&UMTS_TAS_ANT_IDX_by_band, ul1_tas_ant_idx_buffer, sizeof(UMTS_TAS_ANT_IDX_T));
   free_ctrl_buffer(ul1_tas_ant_idx_buffer);
}
#endif
...
#endif/*IS_3G_TAS_NVRAM_SUPPORT*/
#if (IS_3G_TAS_ANTENNA_IDX_ON_TEST_SIM)
        case NVRAM_EF_UL1_TAS_ANT_IDX_TEST_SIM_LID:
            {
               kal_mem_cpy(buffer, &UMTS_TAS_ANT_IDX_by_band, sizeof(UMTS_TAS_ANT_IDX_T));
            }
            break;
