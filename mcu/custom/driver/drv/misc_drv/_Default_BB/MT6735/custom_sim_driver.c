...
void sim_hot_plug_eint_cb_rm_h_common(kal_uint32 idx, SIM_ICC_APPLICATION app)
{
   if (iccHotPlugTable[idx].polarity == LEVEL_HIGH)
   {
      ...
      if (iccHotPlugTable[idx].plugOutcb != NULL)
      {
		...
		iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].removed = KAL_TRUE;
		if (iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].plugOutcb != NULL)
		{
#if defined(__SIM_HOT_SWAP_POLL_TIMER__)
			SIM_PlugEvent_Poll_Timer_Cb(((app <= SIM_ICC_APPLICATION_PHONE2) ? (1-app) : app));
#else
			iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].plugOutcb(((app <= SIM_ICC_APPLICATION_PHONE2) ? (1-app) : app));
#ifdef SIM_HOT_SWAP_V2
			SIM_PlugEvent_Cb(((app <= SIM_ICC_APPLICATION_PHONE2) ? (1-app) : app));
#endif
#endif
		}
#endif	
...
#if defined (__DUAL_SIM_HOT_SWAP_CO_DECK_SUPPORT__)
		iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].polarity = LEVEL_LOW;
#endif
...
#if defined (__DUAL_SIM_HOT_SWAP_CO_DECK_SUPPORT__)
		iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].removed = KAL_FALSE;
		if (iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].plugInCb!= NULL)
		{
			iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].plugInCb(((app <= SIM_ICC_APPLICATION_PHONE2) ? (1-app) : app));
		}
#endif 
	   iccHotPlugTable[idx].polarity = LEVEL_HIGH;	   
#if defined (__DUAL_SIM_HOT_SWAP_CO_DECK_SUPPORT__)
		iccHotPlugTable[((idx<=SIM_ICC_APPLICATION_PHONE2)?(1-idx):idx)].polarity = LEVEL_HIGH;
#endif
