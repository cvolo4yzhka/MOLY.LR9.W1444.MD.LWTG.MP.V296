...
void SP_MuteController() // central controll, and placed between L1Audio_SetFlag and L1Audio_ClearFlag
{
   ...
   kal_trace(TRACE_INFO, L1SP_MUTE_CONTROL_STATUS, 2, l1sp.isULMute, l1sp.isDLMute, l1sp.codec_mute, l1sp.forcedUnmuteDLController, l1sp.forcedUnmuteULController);   
   //DL 
...
#if defined(__SMART_PHONE_MODEM__)	
   // need get enc id first 
   SP_ENC.aud_UL_id = L1Audio_GetAudioID();
   SP_ENC.aud_DL_id = L1Audio_GetAudioID();
#endif   
...
/* under construction !*/
 ...
 #if defined(__SMART_PHONE_MODEM__)		  
   L1Audio_FreeAudioID(SP_ENC.aud_UL_id); 
   L1Audio_FreeAudioID(SP_ENC.aud_DL_id);
 #endif  
}
...
void l1sp_send_codec_status_notify(void *data)//to L4C
{
	 ...
	 if(l1sp.codec_mute){//During ESPEECH OFF, skip all codec notification
	    kal_trace(TRACE_INFO, L1SP_L4C_SEND_CODEC_SKIP, codec);
	    return;    
	 }  
...
void SP_L4C_SetEvent_Execute(L1SP_L4C_Event event, L1SP_L4C_Event_Mode mode)
{
   ...
   kal_trace(TRACE_INFO, L1SP_MUTE_CONTROL_STATUS, 0, l1sp.isULMute, l1sp.isDLMute, l1sp.codec_mute, l1sp.forcedUnmuteDLController, l1sp.forcedUnmuteULController);
   ...
   l1sp_CodecStatusNotification(AM_GetSpeechMode());
   ...
   L1SP_FreeAudID(aud_id);
#endif  
   kal_trace(TRACE_INFO, L1SP_MUTE_CONTROL_STATUS, 1, l1sp.isULMute, l1sp.isDLMute, l1sp.codec_mute, l1sp.forcedUnmuteDLController, l1sp.forcedUnmuteULController);
   }
}
...
{
   kal_trace(TRACE_INFO, L1SP_FORCED_UNMUTE_CONTROLLER, l1sp.forcedUnmuteDLController, mask, b); 
   if(b){  
      l1sp.forcedUnmuteDLController |= mask;
   }else{
      l1sp.forcedUnmuteDLController &= (~mask);
   }
   kal_trace(TRACE_INFO, L1SP_FORCED_UNMUTE_CONTROLLER_1, l1sp.forcedUnmuteDLController); 
...
{
   kal_trace(TRACE_INFO, L1SP_FORCED_UNMUTE_CONTROLLER, l1sp.forcedUnmuteULController, mask, b); 
   if(b){  
      l1sp.forcedUnmuteULController |= mask;
   }else{
      l1sp.forcedUnmuteULController &= (~mask);
   }
   kal_trace(TRACE_INFO, L1SP_FORCED_UNMUTE_CONTROLLER_1, l1sp.forcedUnmuteULController); 
