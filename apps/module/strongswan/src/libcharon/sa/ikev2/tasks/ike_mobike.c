		...
		switch (type)
		{
			case MOBIKE_SUPPORTED:
			{
				...
				else
				{
					...
					{
						DBG1(DBG_IKE, "peer supports MOBIKE refresh NAT");
						/* After tunnel ready send dpd to get latest nat state
						   because of port change after SA_INIT response */
						this->ike_sa->send_mobike_dpd(this->ike_sa);
					}
				}
				break;
			}
	...
	else if (message->get_exchange_type(message) == INFORMATIONAL)
	{
		...
		if (this->check)
		{
			...
			if (this->update)
			{
				...
				this->ike_sa->update_wdrv_keepalive_info(this->ike_sa, TRUE);
				return NEED_MORE;
			}
		}
