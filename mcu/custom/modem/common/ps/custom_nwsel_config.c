...
kal_uint8 ME_STORED_EHPLMNWACT_DATA[ME_STORED_HPLMNWACT_NUM *5] = 
{
  ...
  0x64, 0xf0, 0x40, 0x00, 0x00,/* 460 04, RAT: NONE */
...
/*************************************************************************
* These EHPLMN's are just to know which SIM operator card has been inserted in UE
*************************************************************************/
const char *CUSTOM_RJIO_HPLMN[] = {"405840","405854","405855","405856","405857","405858","405859",
									"405860","405861","405862","405863","405864","405865",
									"405866","405867","405868","405869","405870","405871",
									"405872","405873","405874",CUSTOM_PLMN_SET_END};
const char *CUSTOM_YOIGO_HPLMN[] = {"21404F",CUSTOM_PLMN_SET_END};
...
{
    if (nwsel_query_sbp_feature(SBP_DISABLE_DISPLAY_ROAMING_HPLMN_IN_EPLMN,KAL_FALSE) == KAL_TRUE)
    {
        return KAL_FALSE; //Enable this feature
    }
    else
    {
        return KAL_TRUE; //Feature Disabled
    }
}
...
/*************************************************************************
* Custom Yoigo EQ PLMN when UE is camped on 4G roaming area
* Case A8:
* Yoigo requires UE to reselect to yoigo network when UE is in MANUAL_MODE and 
* user selected PLMN is not HPLMN
*************************************************************************/
char *CUSTOM_4G_EQ_PLMN_YOIGO[] = {"21404F",CUSTOM_EQ_PLMN_SET_END};
kal_uint32 CUSTOM_4G_EQ_PLMN_YOIGO_NUM = sizeof(CUSTOM_4G_EQ_PLMN_YOIGO) /sizeof(char *);
...
const char *CUSTOM_NWSEL_EHPLMN_LIST[][CUSTOM_NWSEL_EHPLMN_MAX_NUM] = {
                                        ...
                                        {"52005F","52018F","FFFFFF"},  //DTAC
                                        {"52000F","52004F","52099F"},  //TRUEMOVE
                                        {"52001F","52003F","52023F"},  //AIS
                                        };
/* How many sets */
...
/*************************************************************************
 * Add custom PLMN's to PLMN_SEARCH list when UE is in MANUAL_MODE
 * Ex:
 * User selected PLMN_A and UE is in MANUAL_MODE
 *  UE lost camping on PLMN_A and UE moved to NO_SERVICE
 *  Later User moved to an area where HPLMN coverage is present.
 *  UE must try searching on HPLMN or any other PLMN defined in this array.
 *  IMPORTANT: PLEASE MAINTAIN 1-1 relation between user selected PLMN (IGNORE_EQPLMN_OPERATOR) and 
 *  ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH arrays.
 * 
 *************************************************************************/
...
{
    kal_uint32 index_eqplmn =0, index_custom_plmn = 0;
    kal_uint32 min_eqplmn_size = 0, min_custom_plmn_size = 0;
    kal_uint32 avail_eqplmn_size = 0, avail_custom_size = 0;
    ...
    {
        const char* YOIGO_IGNORE_EQPLMN_OP[] = {"21403F", "21405F", "21407F", "21404F"};
        
        const char* YOIGO_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH[] = {"21404F", "21404F", "21404F", "21407F"};
    
        kal_uint32 num_eq_plmn = (sizeof(YOIGO_IGNORE_EQPLMN_OP)/sizeof(char *));
        kal_uint32 num_recovery_plmn = (sizeof(YOIGO_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH)/sizeof(char *));


        /***** Any modification to IGNORE_EQPLMN_OPERATOR or ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH 
          should be done before this point *********/

        /* Calculate the updated size */
        IGNORE_EQPLMN_OPERATOR_NUM = sizeof(IGNORE_EQPLMN_OPERATOR)/sizeof(char *);
        ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH_NUM = sizeof(ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH)/sizeof(char *);
        
        for( index_eqplmn = 0; index_eqplmn < IGNORE_EQPLMN_OPERATOR_NUM; index_eqplmn++ )
        {
           /* Search for the first empty location. 
               Assumption is that the first empty location will always have IGNORE_EQPLMN_OPERATOR_SET_END 
             */
           if( strcmp(IGNORE_EQPLMN_OPERATOR[index_eqplmn], IGNORE_EQPLMN_OPERATOR_SET_END) == 0 )
            {
               break;
            }
        }

        for( index_custom_plmn = 0; index_custom_plmn < ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH_NUM; index_custom_plmn++ )
        {
           /* Search for the first empty location. 
               Assumption is that the first empty location will always have IGNORE_EQPLMN_OPERATOR_SET_END 
             */
           if( strcmp(ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH[index_custom_plmn], INVALID_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH) == 0 )
            {
               break;
            }
        }
        ...
        if( index_eqplmn == IGNORE_EQPLMN_OPERATOR_NUM ||
            index_custom_plmn == ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH_NUM )
        {
           /* Memory full */       
           return;
        }

        avail_eqplmn_size = (IGNORE_EQPLMN_OPERATOR_NUM - index_eqplmn);
        avail_custom_size = (ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH_NUM - index_custom_plmn);

        /* Get the minimum of the available size */
        min_eqplmn_size = (avail_eqplmn_size < num_eq_plmn) ? avail_eqplmn_size : num_eq_plmn;
        min_custom_plmn_size  = (avail_custom_size < num_recovery_plmn) ? avail_custom_size : num_recovery_plmn;
        
        if( min_eqplmn_size )
        {
            /* copy as many entries as available space */
            kal_mem_cpy( &(IGNORE_EQPLMN_OPERATOR[index_eqplmn]),
                                YOIGO_IGNORE_EQPLMN_OP,
                                (min_eqplmn_size * sizeof(char*)) 
                              );
           index_eqplmn += min_eqplmn_size;            
         }

         if( min_custom_plmn_size )
         {
            kal_mem_cpy( &(ADD_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH[index_custom_plmn]),
                                    YOIGO_CUSTOM_PLMN_IN_MANUAL_RECOVERY_SEARCH,
                                  (min_custom_plmn_size *  sizeof(char*))
                              );
            index_custom_plmn += min_custom_plmn_size;
         }               
    }
    ...
    if (nwsel_query_sbp_feature(SBP_MM_IGNORE_PLAY_EQPLMN,KAL_FALSE) == KAL_TRUE)
    {
        if (index_eqplmn < IGNORE_EQPLMN_OPERATOR_NUM)
        {
            IGNORE_EQPLMN_OPERATOR[index_eqplmn] = "26006F";
        }
    }
}
...
/*****************************************************************************
* FUNCTION
*  nwsel_custom_search_available_plmn_immediately
* DESCRIPTION
* Some operators wants the SBP to be enabled only for a particular HPLMN/PLMN
*
* PARAMETERS
*  current_sim_interface is passed
* RETURNS
*   KAL_TRUE:
*    - if SBP_SEARCH_AVAILABLE_PLMN_IMMEDIATELY is enabled
*       OR
*       if any of the custom plmn matches the HPLMN.
*   KAL_FALSE:
*    - if SBP_SEARCH_AVAILABLE_PLMN_IMMEDIATELY is disabled
*       AND
*       if none of the custom plmn matches the HPLMN.

* GLOBALS AFFECTED
*  None
*****************************************************************************/
kal_bool nwsel_custom_search_available_plmn_immediately(kal_uint8 sim_interface)
{
    //Please add PLMN's for which this SBP needs to be enabled
    //ex: custom_enabled_plmns[] = {"20826F",CUSTOM_PLMN_SET_END};
    const char * custom_enabled_plmns[] = {CUSTOM_PLMN_SET_END};
    plmn_id_struct custom_plmn;
    kal_uint32 i = 0;
    
    if (nwsel_query_sbp_feature(SBP_SEARCH_AVAILABLE_PLMN_IMMEDIATELY,KAL_TRUE) == KAL_TRUE)
    {
        return KAL_TRUE;
    }
    else
    {
        i = 0;
        //check if any PLMN from custom_enabled_plmns matches
        while (strcmp(custom_enabled_plmns[i], CUSTOM_PLMN_SET_END) != 0)
        {
            if (nas_convert_plmn_string_to_struct(custom_enabled_plmns[i], &custom_plmn) == KAL_TRUE)
            {
                if (nas_is_hplmn(sim_interface, &custom_plmn))
                {
                    return KAL_TRUE;
                }
            }
...
const char *CUSTOM_PS_ONLY_PLMN[] = {"45006F","46011F", /* Adding RJIL PLMNs */
                                     ...
                                     "405873","405874","62127F",
                                      /* Adding Seatel PLMN 45611 */
                                     "45611F", 
                                     "502152", /* Adding Malaysia YES operator 502152 */
                                     "21603F", /* Adding Hungary Digi operator 21603*/
