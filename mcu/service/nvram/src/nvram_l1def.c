...
void  nvram_init_L1_RF_customInputData(void)
{
      ...
      #if defined( __TAS_ANTENNA_IDX_ON_TEST_SIM__)
      {
         l1_tas_init_ant_params_T *table = (l1_tas_init_ant_params_T*)get_ctrl_buffer( sizeof( l1_tas_init_ant_params_T) );
         kal_bool error_no;
         error_no = nvram_external_read_data(NVRAM_EF_L1_TAS_ANT_INIT_PARAMES_LID, 1, (kal_uint8 *)table,   sizeof( l1_tas_init_ant_params_T ) );
         ASSERT(error_no == KAL_TRUE);
         kal_mem_cpy(&L1_TAS_Init_Ant_Params, table , sizeof(l1_tas_init_ant_params_T) );
         free_ctrl_buffer( table );
      }
      #endif
   #endif
