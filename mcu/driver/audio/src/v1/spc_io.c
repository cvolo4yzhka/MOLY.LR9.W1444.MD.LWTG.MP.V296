...
__attribute__  ((section ("NONCACHEDZI"))) kal_uint8 gSpcIo_MDHighPrioBuffer[SPCIO_SHARE_BUF_SIZE];
#endif //__SDIO_DEVICE_CONNECTIVITY__
...
kal_bool spcIO_sendMsgViaCCCI(kal_uint32 msg, kal_uint32 revMsg)
{
   ...
   kal_trace(TRACE_INFO, SPCIO_SEND_M2A_MESSAGE, msg, revMsg);
	...
	if(msgID == SPCIO_MSG_FROM_SPC_UL_ENC_REQUEST ||msgID == SPCIO_MSG_FROM_SPC_DL_DEC_REQUEST )
	{
		 // ENC ccci msg run in CSR/L1CSR task,can't call kal_sleep_task
...
AUD_CCCI_MSG_T spcIO_msgMapper(SPCIO_MSG_FROM_SPC spcIoMsg)
{
	...
	switch(spcIoMsg){
		...
		*/
		...
		case SPCIO_MSG_FROM_ECALL_CTL_SEQ_SWITCH_ACK: ccciMsgCmd = MSG_M2A_ECALL_CTL_SEQ_SWITCH; break;
		case SPCIO_MSG_FROM_ECALL_HANDSHAKE_INFO_ACK: ccciMsgCmd = MSG_M2A_ECALL_HANDSHAKE_INFO_ACK; break;
		...
		case SPCIO_MSG_FROM_SPC_UL_ENC_REQUEST: ccciMsgCmd = MSG_M2A_ECALL_UL_ENC_REQUEST; break;
		case SPCIO_MSG_FROM_SPC_DL_DEC_REQUEST: ccciMsgCmd = MSG_M2A_ECALL_DL_ECC_REQUEST; break;
		case PCIO_MSG_FROM_SPC_ECALL_TX_CTRL_PAR_ACK: ccciMsgCmd = MSG_M2A_ECALL_TX_CTRL_PAR_ACK; break;
		case PCIO_MSG_FROM_SPC_ECALL_RX_CTRL_PAR_NOTIFY: ccciMsgCmd = MSG_M2A_ECALL_RX_CTRL_PAR_NOTIFY; break;
