...
/* under construction !*/
            ...
            /* MD1 in ASYNC state */
            else if (cc_sys_state == CC_IRQ_SYS_STATE_ASYNC)
            {
                if (sys_hdr->ctrl_type == CC_IRQ_SYS_PKT_TYPE_INIT_HS)
                {/* MD3 reset independently */
                    EXT_ASSERT(0, cc_sys_state, sys_hdr->ctrl_type, ust_get_current_time());
                }
                /* Ignore PING ACK */
                else
                {
