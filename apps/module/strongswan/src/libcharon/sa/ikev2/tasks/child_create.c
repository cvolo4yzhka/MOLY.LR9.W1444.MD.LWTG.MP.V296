	...
	private_child_create_t *this, message_t *message)
{
    ...
    if (this->rekey)
    {
        /*store traffic selector for next rekey*/
        if (this->child_sa)
        {
            this->child_sa->set_rekey_tsi(this->child_sa, this->tsi);
            DBG1(DBG_IKE, "store tsi for rekey");
            this->child_sa->set_rekey_tsr(this->child_sa, this->tsr);
            DBG1(DBG_IKE, "store tsr for rekey");
        }
	...
	private_child_create_t *this, message_t *message)
{
	...
	if (select_and_install(this, no_dh, ike_auth) == SUCCESS)
	{
		...
		if (this->ike_sa->has_condition(this->ike_sa, COND_REAUTHENTICATING)) {
			this->ike_sa->set_condition(this->ike_sa, COND_REAUTHENTICATING, FALSE);
		}
		else if (!this->rekey)
		{	/* invoke the child_up() hook if we are not rekeying */
