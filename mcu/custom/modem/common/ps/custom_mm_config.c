...
/*****************************************************************************
* FUNCTION
*  mm_custom_cs_ps_reject_cause_convert
*
* DESCRIPTION
*    To convert LAU/Attach/RAU reject cause based on HPLMN and VPLMN 
*     
*  
* 
*
* PARAMETERS
*    reject cause : Reject cause value from Network
*    on_hplmn :Determine whether UE camped on HPLMN or not 
*    mm_proc : Determine the MM/GMM procedure
*    sim_index : sim_interface
* 
RETURNS
*   enum : converted reject cause 
*   
*    
*
* GLOBALS AFFECTED
*  None
*****************************************************************************/
mm_custom_reject_cause mm_custom_cs_ps_reject_cause_transform
                        (mm_custom_reject_cause reject_cause ,kal_bool on_hplmn, mm_custom_mm_proc mm_proc,
                         mm_custom_sim_index sim_index)
         
{
    mm_custom_reject_cause cust_reject_cause = reject_cause;
	 if (on_hplmn == KAL_TRUE)
	 {
	     /*Camped on HPLMN, Reject cause transform for LAU /Attach /RAU */
	     switch (reject_cause)
         {
                case MM_CUST_IMSI_UNKNOWN_IN_HLR: /*0x2 */
			    case MM_CUST_ILLEGAL_MS: /*0x3 */
			    case MM_CUST_ILLEGAL_ME: /*0x6 */
			    case MM_CUST_GPRS_NOT_ALLOWED: /*0x7 */
			    case MM_CUST_GPRS_NON_GPRS_NOT_ALLOWED : /*0x8*/
			        cust_reject_cause = MM_CUST_NETWORK_FAILURE; /* 0x17*/
                    break;
                default:
                    break;
         }
	   ...
	   switch (reject_cause)
         {
               case MM_CUST_IMSI_UNKNOWN_IN_HLR: /*0x2 */
					 /* only  LAU reject */
                    if (mm_proc == MM_CUST_PROC_LU) 
					{
				        cust_reject_cause = MM_CUST_ROAMING_AREA_NOT_ALLOWED;
					}
                    break;
			   case MM_CUST_ILLEGAL_MS: /*0x3 */
					cust_reject_cause = MM_CUST_ROAMING_AREA_NOT_ALLOWED; /* reject cause #13 */
					break;
			   ...
			   case MM_CUST_ILLEGAL_ME: /*0x6 */
					 ...
					 {
                         /*for Attach/RAU reject */
                         cust_reject_cause = MM_CUST_GPRS_NOT_ALLOWED_IN_PLMN; /*reject cause #14 */
					 }   
					break;
			   case MM_CUST_GPRS_NOT_ALLOWED: /*0x7 */
					 /*for Attach/RAU reject */
					 if((mm_proc == MM_CUST_PROC_ATTACH) || (mm_proc == MM_CUST_PROC_RAU) ) 
					 {
					   cust_reject_cause = MM_CUST_GPRS_NOT_ALLOWED_IN_PLMN;
					 }
					break; 
			   case MM_CUST_GPRS_NON_GPRS_NOT_ALLOWED :/*0x8*/
					 /*for Attach/RAU reject */
...
*  is_test      - if test-sim 
*  sim_id  - Protocol stack ID of current operating Stack
* RETURNS
*  kal_uint8
*
* GLOBALS AFFECTED
...
*
*****************************************************************************/
const char *CONFIGURE_SKIP_LU_AFTER_CSFB_FOR_EAFR_PLMN[] = {"25002F","71006F","71010F","71017F","722310", CONFIGURE_SKIP_LU_AFTER_CSFB_FOR_EAFR_PLMN_END};
