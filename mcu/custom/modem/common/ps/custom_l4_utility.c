...
{
    if ((sml_query_sml_lock_rule() == SML_LOCK_RULE_LINK_LOCK_BY_SIM1_SLOT) ||
        (sml_query_sml_lock_rule() == SML_LOCK_RULE_LINK_LOCK_BY_SIM2_SLOT))
    {
        sml_update_legal_service(SML_RULE_FULL_SERVICE, SML_STATUS_SIM_VALID, target_sim_protocol);
    }
    else if (sml_query_linksml_invalid_sim_rule() == SML_LOCK_SUB_RULE_LS_FULL_SERVICE)
    {
        sml_update_legal_service(SML_RULE_FULL_SERVICE, SML_STATUS_SIM_INVALID, target_sim_protocol);
    }
    else if (sml_query_linksml_invalid_sim_rule() == SML_LOCK_SUB_RULE_LS_CS_ONLY_SERVICE)
    {
...
link_sml_check_enum custom_ckeck_is_valid_sim_exist_by_rule(kal_uint8 updated_sim_slot)
{
    ...
    if (sml_query_sml_lock_rule() == SML_LOCK_RULE_LINK_LOCK_BY_ALL_SIM_SLOT)
    {
        ...
        for (i = 0; i < MAX_SIM_NUM; i++)
        {
            if (i != updated_sim_slot)
            {
                ...
                if (sml_query_legal_service(target_sim_protocol) == SML_RULE_FULL_SERVICE)
                {
                    ...
                    return LINK_SML_CHECK_VALID_SIM_EXIST;
                }
                else if (sml_query_legal_service(target_sim_protocol) == SML_RULE_UNKNOWN)
                {
                    ...
                    kal_prompt_trace(kal_get_active_module_id(),
                                     ...
                                     target_sim_protocol, 
                                     i);
                    return LINK_SML_CHECK_WAITING;
                }
            }
        }
    }
    else if (sml_query_sml_lock_rule() == SML_LOCK_RULE_LINK_LOCK_BY_SIM1_SLOT)
    {
        ...
        if (SIM1 == updated_sim_slot)
        {
            ...
            kal_prompt_trace(kal_get_active_module_id(),
                             "[LINK_SML] SIM%d(Slot1) is Invalid SIM", 
                             target_sim_protocol);
        ...
        if (sml_query_legal_service(target_sim_protocol) == SML_RULE_FULL_SERVICE)
        {
            ...
            kal_prompt_trace(kal_get_active_module_id(),
                             "[LINK_SML] SIM%d(Slot1) is Valid SIM", 
                             target_sim_protocol);
        ...
        else if (sml_query_legal_service(target_sim_protocol) == SML_RULE_UNKNOWN)
        {
            /* SIM1 slot is still waiting verification result,
               return and update current card to BUSY */
            kal_prompt_trace(kal_get_active_module_id(),
                             "[LINK_SML] SIM%d(Slot1) is still BUSY", 
                             target_sim_protocol);

            return LINK_SML_CHECK_WAITING;
        }
    }
    else if (sml_query_sml_lock_rule() == SML_LOCK_RULE_LINK_LOCK_BY_SIM2_SLOT)
    {
        ...
        if (SIM2 == updated_sim_slot)
        {
            /* SIM2 slot take control but Invalid SIM inserted,
               change all SIM slot to Invalid SIM */
            kal_prompt_trace(kal_get_active_module_id(),
                             "[LINK_SML] SIM%d(Slot2) is Invalid SIM", 
                             target_sim_protocol);
        ...
        if (sml_query_legal_service(target_sim_protocol) == SML_RULE_FULL_SERVICE)
        {
            ...
            kal_prompt_trace(kal_get_active_module_id(),
                             "[LINK_SML] SIM%d(Slot2) is Valid SIM", 
                             target_sim_protocol);
        ...
        else if (sml_query_legal_service(target_sim_protocol) == SML_RULE_UNKNOWN)
        {
            /* SIM2 slot is still waiting verification result,
               return and update current card to BUSY */
            kal_prompt_trace(kal_get_active_module_id(),
                             "[LINK_SML] SIM%d(Slot2) is still BUSY", 
                             target_sim_protocol);
...
{
    if (((mcc == 262) && (mnc == 01)) ||      // Germany
        ((mcc == 262) && (mnc == 06)) ||      // Germany
        ((mcc == 260) && (mnc == 02)) ||      // Poland
        ((mcc == 260) && (mnc == 34)) ||      // Poland
        ((mcc == 230) && (mnc == 01)) ||      // Czech
        ((mcc == 202) && (mnc == 01)) ||      // DTAG-Greece
        ((mcc == 202) && (mnc == 02)) ||      // DTAG-Greece
        // TODO: MOLY00269213 has mapped these two PLMNs to wrong SBP id, need to correct them when hit issue.
        // ((mcc == 204) && (mnc == 16)) ||   // Netherlands/DTAG
        // ((mcc == 204) && (mnc == 20)) ||   // Netherlands/DTAG
        ((mcc == 216) && (mnc == 30)))        // DTAG Hungary
    {
        return 5;   // DTAG(TMOEU)
    }
    else if (((mcc == 440) && (mnc == 10)) ||
             ((mcc == 310) && (mnc == 370)) ||
             ((mcc == 310) && (mnc == 470)))
    {
        return 17;  // DOCOMO
    }
    else if ( ((mcc == 404) && (mnc == 02)) ||
         ((mcc == 404) && (mnc == 03)) ||
         ((mcc == 404) && (mnc == 10)) ||
         ((mcc == 404) && (mnc == 16)) ||
         ((mcc == 404) && (mnc == 31)) ||
    ...
    else if ((mcc == 520) && (mnc == 03))
    {
        return 122; // AIS
    }
    else if ( ((mcc == 238) && (mnc == 01)) ||
              ((mcc == 238) && (mnc == 10)) )
    {
        return 175; // Denmark TDC
    }
              ...
              ((mcc == 466) && (mnc == 90)) )
    {
        return 176; // T-star
              ...
              ((mcc == 204) && (mnc == 20)) )
    {
        return 8; // Telekom (T-Moble) Netherlands
    }
	else if ((mcc == 204) && (mnc == 02))
    {
        return 137; // Tele2 Netherlands
    }
	else if ((mcc == 246) && (mnc == 01))
    {
        return 154; // Telia Lithuania
    }
#if defined(__UMTS_FDD_CPC_OFF__) 
    else if (((mcc == 214) && (mnc == 07)) )
    {
        return 15;   // Telefonica
    }
#endif	
...
*     custom_cc_timer()
* DESCRIPTION
*     This is the function to return the customized CC timer values 
* PARAMETERS
*     
* RETURNS
