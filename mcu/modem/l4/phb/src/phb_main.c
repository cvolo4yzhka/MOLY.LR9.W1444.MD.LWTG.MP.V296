...
extern void phb_primitive_mismatch_err_handler(control_block_type *control_block);
/*****************************************************************************
 * FUNCTION
 *  phb_main
 * DESCRIPTION
    ...
    else if (ilm_ptr->msg_id == MSG_ID_L4CPHB_INIT_LN_REQ)
    {
	  if (phb_ptr->state != PHB_STATE_NULL)
	  {
		...
		for (i = 0; i < PHB_CONCURRENCY_DEGREE; ++i)
		{
			...
			if (control_block->is_allocated == KAL_TRUE && control_block->res_msg_id != 0xFF)
			{	
				kal_trace(TRACE_INFO, INFO_PHB_L4C_WRONG_RES_IF_CB, control_block->ID, control_block->res_msg_id, control_block->cnf_msg_id);
				kal_trace(TRACE_ERROR, ERROR_PHB_ABNORMAL);
				phb_primitive_mismatch_err_handler(control_block);
	  		}
    	}
	  }
...
/*****************************************************************************
 * FUNCTION
 *  phb_primitive_mismatch_err_handler
 * DESCRIPTION
 *  This is API to use when L4C SIM IO REQ has not completed meanwhile
 *  new startup request has come, then send SIM CNF to L4C with error.
 * PARAMETERS
 *  void
 * RETURNS
 *  True if succeed.
 *****************************************************************************/
void phb_primitive_mismatch_err_handler(control_block_type *control_block)
{
	...
	switch (control_block->cnf_msg_id)
	{
		case MSG_ID_L4CPHB_READ_REQ:
		{
			...
			l4cphb_read_cnf->total = control_block->actual_count;
			l4cphb_read_cnf->src_id = control_block->src_id;
			l4cphb_read_cnf->result = PHB_ERRNO_FAIL;
			l4cphb_read_cnf->cause = SIM_CMD_FAIL;
			local_param_ptr = (local_para_struct*) l4cphb_read_cnf;
			phb_send_ilm(MOD_L4C, (kal_uint16) MSG_ID_L4CPHB_READ_CNF, local_param_ptr, NULL);
			break;
		}	
		case MSG_ID_L4CPHB_READ_USIM_REQ:
		{
		   ...
		   l4cphb_read_cnf->cause = SIM_CMD_FAIL;
		   local_param_ptr = (local_para_struct*) l4cphb_read_cnf;
		   phb_send_ilm(MOD_L4C, (kal_uint16) MSG_ID_L4CPHB_READ_USIM_CNF, local_param_ptr, NULL);
		   break;
		}  			
		case MSG_ID_L4CPHB_WRITE_USIM_REQ:
		{
			...
			l4cphb_write_cnf->result = PHB_ERRNO_FAIL;
			...
			l4cphb_write_cnf->cause = SIM_CMD_FAIL;
		...
		case MSG_ID_L4CPHB_DELETE_USIM_REQ:
		{
		   ...
		   l4cphb_delete_cnf->result = PHB_ERRNO_FAIL;
		   ...
		   l4cphb_delete_cnf->cause = SIM_CMD_FAIL;
