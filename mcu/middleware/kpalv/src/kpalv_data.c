...
#else
    result = ipc_send_ul_pkt_by_pdn(&ipc_pkt, NULL, keep_alive_cntx[conn_id].control_info.pdn_id, ip_type);
#endif
    if (KAL_FALSE == result) {
...
void kpalv_start_idle_timer(kal_uint8 conn_id)
{
    ...
    kpalv_cntx->control_info.eid_idle_timer = evshed_set_event(kpalv_cntx->control_info.idle_timer_scheduler,
            (kal_timer_func_ptr)kpalv_idle_poll_tmout_hndlr,
             kpalv_cntx, 
             kpalv_cntx->at_info.idle_time*(10* KAL_TICKS_100_MSEC)
             );
...
void kpalv_start_retry_timer(kal_uint8 conn_id)
{
    ...
    kpalv_cntx->control_info.eid_retry_timer  = evshed_set_event(kpalv_cntx->control_info.retry_timer_scheduler,
        (kal_timer_func_ptr)kpalv_interval_poll_tmout_hndlr,
         kpalv_cntx, 
         kpalv_cntx->at_info.probe_interval*(10* KAL_TICKS_100_MSEC)
         );
...
{
    imsp_kpalv_keepalive_req_struct *at_cmd = (imsp_kpalv_keepalive_req_struct *)(ilm->local_para_ptr);
...
{   
    switch (ilm->msg_id) {
        case MSG_ID_IMSP_KPALV_KEEPALIVE_REQ:
...
void kpalv_register_ul_filter_at_ipc(kal_uint8 conn_id, kpalv_req_ip_type_e ip_type)
{
    ...
    //clone feature not supported at ipcore , for natt only this will not required
                        ...
                        kal_uint32 length)
{
    ...
    /* pass to AP via IPCORE*/
    {
        ...
        netif_id = keep_alive_cntx[conn_id].at_info.netif_id | IPC_NETIF_ID_CCMNI_BEGIN;
...
{      
    //MSG_ID_IMSP_KPALV_CONNECTION_STATUS_IND
    imsp_kpalv_connection_status_ind_struct *p_ind = NULL;
    kal_uint16 sap_id = KPALV_SAP;

    p_ind = (imsp_kpalv_connection_status_ind_struct *)construct_local_para
                    (sizeof(imsp_kpalv_connection_status_ind_struct), TD_RESET);
    ...
    msg_send6(MOD_KPALV, MOD_IMSP + rsp_ps_idx, sap_id, MSG_ID_IMSP_KPALV_CONNECTION_STATUS_IND,
...
void kpalv_send_keep_alive_notify(kal_bool ok, kal_uint8 conn_handle, kpalv_md_kpalive_state_e state, kal_bool ap_dis_flow, kal_uint8 rsp_ps_idx)
{
    ...
    //MSG_ID_IMSP_KPALV_KEEPALIVE_CNF
    imsp_kpalv_keepalive_cnf_struct *p_ind = NULL;
    kal_uint16 sap_id = KPALV_SAP;

    p_ind = (imsp_kpalv_keepalive_cnf_struct *)construct_local_para
                    (sizeof(imsp_kpalv_keepalive_cnf_struct), TD_RESET);
        ...
        msg_send6(MOD_KPALV, MOD_IMSP + rsp_ps_idx, sap_id, MSG_ID_IMSP_KPALV_KEEPALIVE_CNF,
                     (local_para_struct *)p_ind, NULL);
}
...
{   
    ...
    kal_uint8 atp_idx = ilm->src_mod_id - MOD_IMSP;
    imsp_kpalv_keepalive_req_struct *at_cmd = (imsp_kpalv_keepalive_req_struct *)(ilm->local_para_ptr);
    ...
    if (at_cmd->parsing_result == KAL_TRUE) {
        ...
        netif_id = at_cmd->netif_id | IPC_NETIF_ID_CCMNI_BEGIN;
