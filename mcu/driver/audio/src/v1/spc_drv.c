...
#if defined(__ECALL_SUPPORT__)		
			spc_eCall_TX_CTRL_Data((kal_uint16)msgData32, msgData16);
			SpcIO_SendMsgToAp(PCIO_MSG_FROM_SPC_ECALL_TX_CTRL_PAR_ACK, 0, 0);
#endif			
			break;		
...
void Spc_VoiceEncryptionSwitch(bool enable)
{
    if(enable)
    {
        ...
        if((AM_GetNetworkRate() == RAT_2G_MODE) && (AM_IsAmInSpeechState())) 
				{
					SP_ENC_2G_Open();
				}
   	 }
...
void spc_Set_2GULDataEnc(void)
{
	...
	info.length = (UL_2G_ENC_MAX_DATA_SIZE*2);

	kal_trace( TRACE_FUNC,SPC_ENC_SET_2G_UL_DATA_ENC,SP_ENC.UL_2G_befor_Enc_buffer[0],SP_ENC.UL_2G_befor_Enc_buffer[1],SP_ENC.UL_2G_befor_Enc_buffer[2],SP_ENC.UL_2G_befor_Enc_buffer[3],SP_ENC.UL_2G_befor_Enc_buffer[4],SP_ENC.UL_2G_befor_Enc_buffer[5],SP_ENC.UL_2G_befor_Enc_buffer[6]);
	sendResult = SpcIO_WriteDataToAp(&info, sizeof(spcBufInfo), 
             SP_ENC.UL_2G_befor_Enc_buffer, info.length, NULL, 0,
...
void spc_Set_2GDLDataDec(void)
{
	...
	info.length = (DL_2G_DEC_MAX_DATA_SIZE*2);

	SP_ENC.Enc_curr_frc = ust_get_current_time();
	kal_trace( TRACE_FUNC,SPC_ENC_SET_2G_DL_DATA_DEC,SP_ENC.DL_2G_befor_Dec_buffer[0],SP_ENC.DL_2G_befor_Dec_buffer[1],SP_ENC.DL_2G_befor_Dec_buffer[2],SP_ENC.DL_2G_befor_Dec_buffer[3],SP_ENC.DL_2G_befor_Dec_buffer[4],SP_ENC.DL_2G_befor_Dec_buffer[5],SP_ENC.DL_2G_befor_Dec_buffer[6]);
    sendResult = SpcIO_WriteDataToAp(&info, sizeof(spcBufInfo), 
             SP_ENC.DL_2G_befor_Dec_buffer, info.length, NULL, 0,
...
void Spc_GetVoiceEncryptionHandler(kal_uint16 offset, kal_uint16 length)
{
	...
	spcBufInfo info;
	spcEnCInfo EncInfo;
	spc2GEnCInfo Enc2GInfo;
    if( SP_ENC.Is_Voice_Encryption == false || (!AM_IsAmInSpeechState()) ){ 	
		return;
	}
	...
	if(SP_ENC.Is_2G_Voice_Enc)
	{
		curOffSet = SpcIo_GetDataFromAp(curOffSet, sizeof(spc2GEnCInfo), &Enc2GInfo);
		kal_trace( TRACE_FUNC,SPC_ENC_GET_UL_DATA,Enc2GInfo.network,Enc2GInfo.count,(info.length-sizeof(spc2GEnCInfo)));
		if(Enc2GInfo.network == Set_Network_2G)
		{
			if( SP_ENC.UL_2G_after_Enc_buffer_count == MAX_UL_2G_ENC_BUFFER_NUM )
   			{
   				SP_ENC.UL_2G_after_Enc_buffer_read_index++;          //drop oldest frame
			...
			SpcIo_GetDataFromAp(curOffSet, (info.length-sizeof(spc2GEnCInfo)), SP_ENC.UL_2G_after_Enc_buffer[SP_ENC.UL_2G_after_Enc_buffer_write_index]);	
			
			SP_ENC.UL_2G_after_Enc_buffer_write_index++;
			if(SP_ENC.UL_2G_after_Enc_buffer_write_index== MAX_UL_2G_ENC_BUFFER_NUM)
			{
				SP_ENC.UL_2G_after_Enc_buffer_write_index = 0;
			}
			SP_ENC.UL_2G_after_Enc_buffer_count++;
            ...
            kal_trace( TRACE_FUNC,SPC_ENC_CHECK_2G_UL_BUFFER_INDEX,0,SP_ENC.UL_2G_after_Enc_buffer_count,SP_ENC.UL_2G_after_Enc_buffer_read_index,SP_ENC.UL_2G_after_Enc_buffer_write_index);
			
      	}
		else
		{
            kal_trace( TRACE_FUNC,SPC_ENC_DROP_OLDEST_FRAME,3);
		}
	...
	else
	{
    	...
    	{			
			...
			case Set_Network_4G_AMR:
			{
            	...
            	kal_trace( TRACE_FUNC,SPC_ENC_CHECK_4G_AMR_UL_BUFFER_INDEX,2,SP_ENC.UL_4G_AMR_after_Enc_buffer_count,SP_ENC.UL_4G_AMR_after_Enc_buffer_read_index,SP_ENC.UL_4G_AMR_after_Enc_buffer_write_index);
      		...
      		default:
			{
				kal_trace( TRACE_FUNC,SPC_ENC_DROP_UNKNOWN_NETWORK_DATA,0,EncInfo.network);
      		}
         		break;
   		}
	}
...
void Spc_GetVoiceDecryptionHandler(kal_uint16 offset, kal_uint16 length)
{
	...
	spcBufInfo info;
	spcEnCInfo DecInfo;
	spc2GEnCInfo Dec2GInfo;
	
	SP_ENC.Enc_latest_us = ust_get_current_time();
	SP_ENC.Enc_delay_time = ust_us_duration(SP_ENC.Enc_curr_frc, SP_ENC.Enc_latest_us);
	kal_trace( TRACE_FUNC,SPC_ENC_CHECK_DELAY_TIME,SP_ENC.Enc_delay_time);
    ...
    if(SP_ENC.Is_2G_Voice_Enc)
	{
		curOffSet = SpcIo_GetDataFromAp(curOffSet, sizeof(spc2GEnCInfo), &Dec2GInfo);
		kal_trace( TRACE_FUNC,SPC_DEC_GET_DL_DATA,Dec2GInfo.network,Dec2GInfo.count,(info.length-sizeof(spc2GEnCInfo)));
		if(Dec2GInfo.network == Set_Network_2G)
		{
            if( SP_ENC.DL_2G_after_Dec_buffer_count == MAX_DL_2G_ENC_BUFFER_NUM )
   			{
   				SP_ENC.DL_2G_after_Dec_buffer_read_index++;          //drop oldest frame
			...
			SpcIo_GetDataFromAp(curOffSet, (info.length-sizeof(spc2GEnCInfo)), SP_ENC.DL_2G_after_DEC_buffer[SP_ENC.DL_2G_after_Dec_buffer_write_index]);	
			
			SP_ENC.DL_2G_after_Dec_buffer_write_index++;
			if(SP_ENC.DL_2G_after_Dec_buffer_write_index== MAX_DL_2G_ENC_BUFFER_NUM)
			{
				SP_ENC.DL_2G_after_Dec_buffer_write_index = 0;
			}
			SP_ENC.DL_2G_after_Dec_buffer_count++;
            kal_trace( TRACE_FUNC,SPC_DEC_CHECK_2G_DL_BUFFER_INDEX,0,SP_ENC.DL_2G_after_Dec_buffer_count,SP_ENC.DL_2G_after_Dec_buffer_read_index,SP_ENC.DL_2G_after_Dec_buffer_write_index);
		
      	}
		else
		{
            kal_trace( TRACE_FUNC,SPC_ENC_DROP_OLDEST_FRAME,4);
		}
	...
	else
	{
		...
		{	
			...
			case Set_Network_4G_AMR:
			{
      		...
      		default:
			{
				kal_trace( TRACE_FUNC,SPC_ENC_DROP_UNKNOWN_NETWORK_DATA,0,DecInfo.network);
      		}
			break;
   		}
	}
...
void spc_eCall_SDT_Open(void)
{
	...
	eCall_IVS_PutMSD(eCall_Msd,140); // DEFAULT PAR
}
...
void spc_eCall_IVS_Open(void)
{
	...
	eCall_IVS_Open(aud_ecall_event_callback);
	eCall_IVS_PutMSD(eCall_Msd,140);
}
...
void spc_eCall_Msd_Data(kal_uint16 offset, kal_int16 length)
{
	...
	curOffSet = SpcIo_GetDataFromAp(offset, sizeof(spcBufInfo), &info);
         ...
         //ASSERT(info.length == (length - sizeof(spcBufInfo)));
        SpcIo_GetDataFromAp(curOffSet, 140, eCall_Msd);
        eCall_IVS_PutMSD(eCall_Msd,140);
}
...
void spc_eCall_TX_CTRL_Data(kal_uint16 offset, kal_int16 length)
{
	...
	curOffSet = SpcIo_GetDataFromAp(offset, sizeof(spcBufInfo), &info);
	ASSERT(info.syncWord == 0xA2A2);
         ASSERT(info.type == AUD_CCCI_ECALL_TX_CTRL_SEQ_TYPE);
	 //ASSERT(info.length == (length - sizeof(spcBufInfo)));
          memset(eCall_TX_Temp, 0, 800*sizeof(short));
	 curOffSet = SpcIo_GetDataFromAp(curOffSet, 1600, &eCall_TX_Temp);

          kal_trace( TRACE_FUNC,SPC_ECALL_CHECK_TX_CTRL_HEADER,eCall_TX_Temp[0]);
	 if(eCall_TX_Temp[0] == 0xAA)
	 {
		 ...
		 for(i=0;i<30;i++)
		 {
			...
			kal_trace( TRACE_GROUP_ECALL,ECALL_TX_CTRL_STATE,i*5,eCall_TX_CTRL_SEQ_State[i*5],i*5+1,eCall_TX_CTRL_SEQ_State[i*5+1],i*5+2,eCall_TX_CTRL_SEQ_State[i*5+2],i*5+3,eCall_TX_CTRL_SEQ_State[i*5+3],i*5+4,eCall_TX_CTRL_SEQ_State[i*5+4]);
		 }
	 }
	 else if(eCall_TX_Temp[0] == 0xBB)
	{
		...
		memcpy(eCall_TX_CTRL_SEQ_dlData, eCall_TX_Temp, 800*sizeof(int16));
		  for(i=0;i<30;i++)
		 {
		 	kal_trace( TRACE_GROUP_ECALL,ECALL_TX_CTRL_DATA,i*5,eCall_TX_CTRL_SEQ_dlData[i*5],i*5+1,eCall_TX_CTRL_SEQ_dlData[i*5+1],i*5+2,eCall_TX_CTRL_SEQ_dlData[i*5+2],i*5+3,eCall_TX_CTRL_SEQ_dlData[i*5+3],i*5+4,eCall_TX_CTRL_SEQ_dlData[i*5+4]);
		 }
	}
	 else if(eCall_TX_Temp[0] == 0xCC)
	 {
		 ...
		 for(i=0;i<30;i++)
		 {
		 	kal_trace( TRACE_GROUP_ECALL,ECALL_TX_CTRL_METRIC,i*5,eCall_TX_CTRL_SEQ_dlMetric[i*5],i*5+1,eCall_TX_CTRL_SEQ_dlMetric[i*5+1],i*5+2,eCall_TX_CTRL_SEQ_dlMetric[i*5+2],i*5+3,eCall_TX_CTRL_SEQ_dlMetric[i*5+3],i*5+4,eCall_TX_CTRL_SEQ_dlMetric[i*5+4]);
		 }
	 }
...
void Set_eCall_Par_Crtl_Switch(kal_uint8 Switch)
{
	...
	kal_trace(TRACE_GROUP_ECALL,ECALL_IVS_SEND_PAR_CTRL_SWITCH, gSpc.eCall_Ctrl_Seq_Switch);
}
...
void spc_eCall_RX_CTRL_Data(uint16 header)
{
	...
	bool sendResult = true; 
	info.syncWord = 0x2A2A;
	info.type = AUD_CCCI_ECALL_RX_CTRL_SEQ_TYPE; 
	info.length = 1600;
	uint32 i;
	...
	if(header == 0xDD )
	{
		sendResult = SpcIO_WriteDataToAp(&info, sizeof(spcBufInfo), eCall_RX_CTRL_SEQ_State, info.length, NULL, 0,PCIO_MSG_FROM_SPC_ECALL_RX_CTRL_PAR_NOTIFY); 
		for(i=0;i<30;i++)
		 {
		 	kal_trace( TRACE_GROUP_ECALL,ECALL_RX_CTRL_STATE,i*5,eCall_RX_CTRL_SEQ_State[i*5],i*5+1,eCall_RX_CTRL_SEQ_State[i*5+1],i*5+2,eCall_RX_CTRL_SEQ_State[i*5+2],i*5+3,eCall_RX_CTRL_SEQ_State[i*5+3],i*5+4,eCall_RX_CTRL_SEQ_State[i*5+4]);
		 }
	}
	else if(header == 0xEE)
	{
		sendResult = SpcIO_WriteDataToAp(&info, sizeof(spcBufInfo), eCall_RX_CTRL_SEQ_dlData, info.length, NULL, 0,PCIO_MSG_FROM_SPC_ECALL_RX_CTRL_PAR_NOTIFY); 
		for(i=0;i<30;i++)
		 {
		 	kal_trace( TRACE_GROUP_ECALL,ECALL_RX_CTRL_DATA,i*5,eCall_RX_CTRL_SEQ_dlData[i*5],i*5+1,eCall_RX_CTRL_SEQ_dlData[i*5+1],i*5+2,eCall_RX_CTRL_SEQ_dlData[i*5+2],i*5+3,eCall_RX_CTRL_SEQ_dlData[i*5+3],i*5+4,eCall_RX_CTRL_SEQ_dlData[i*5+4]);
		 }
	}
	else if (header == 0xFF)
	{	
		sendResult = SpcIO_WriteDataToAp(&info, sizeof(spcBufInfo), eCall_RX_CTRL_SEQ_dlMetric, info.length, NULL, 0,PCIO_MSG_FROM_SPC_ECALL_RX_CTRL_PAR_NOTIFY); 
		for(i=0;i<30;i++)
		 {
		 	kal_trace( TRACE_GROUP_ECALL,ECALL_RX_CTRL_METRIC,i*5,eCall_RX_CTRL_SEQ_dlMetric[i*5],i*5+1,eCall_RX_CTRL_SEQ_dlMetric[i*5+1],i*5+2,eCall_RX_CTRL_SEQ_dlMetric[i*5+2],i*5+3,eCall_RX_CTRL_SEQ_dlMetric[i*5+3],i*5+4,eCall_RX_CTRL_SEQ_dlMetric[i*5+4]);
		 }
	}
...
{
	kal_trace(TRACE_GROUP_ECALL,ECALL_IVS_ECALL_HANDSHAKE_INFO_NOTIFY, header,data);
