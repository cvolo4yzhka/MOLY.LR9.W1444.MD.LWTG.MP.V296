...
volatile uint16* SALI_GetFltCoefAddr(uint8 type)
{
	...
	switch(type)
	{
		...
		case SALI_FLTCOEF_TYPE_NOTCH:
			...
			break;
		case SALI_FLTCOEF_TYPE_ECHO_REF_SW_BT:
			addr = DSP_DM_ADDR(7, 0x0000);
			break;	
...
bool SALI_Int_Resolve(uint16 iid, uint32* sph_int)
{
	...
	*sph_int = val;
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(16), iid, val, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);
	return from_sph;
...
void SALI_HNW_Switch(bool on)
{
	if(on)
	{
		*DSP_SPH_HNW_CTRL |= 0x1;
		*DSP_SPH_SCH_IMPROVE_CTRL &= ~0x4000; //Enable DSH_2G_UL_CRC
	}
	else
	{
		*DSP_SPH_HNW_CTRL &= ~0x0001;	
		*DSP_SPH_SCH_IMPROVE_CTRL |=  0x4000; //Disable DSH_2G_UL_CRC
	}

	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(17), *DSP_SPH_HNW_CTRL, 0, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);
}

void SALI_HNW_Set_Ready_UL(bool on)
{
	if(on)
		*DSP_SPH_HNW_CTRL |= 0x2;	
	else
		*DSP_SPH_HNW_CTRL &= ~0x0002;	
		
	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(17), *DSP_SPH_HNW_CTRL, 1, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);		
}

void SALI_HNW_Set_Ready_DL(bool on)
{
	if(on)
		*DSP_SPH_HNW_CTRL |= 0x4;
	else
		*DSP_SPH_HNW_CTRL &= ~0x0004;	

	L1Audio_Msg_SAL_SET_VALUE_MULTI(SAL_DSP_VALUE_NAME(17), *DSP_SPH_HNW_CTRL, 2, SAL_UNKNOWN, SAL_UNKNOWN, SAL_UNKNOWN);		
}

void SALI_Get_2G_Data(Sal_2G_Data *GSM_data)
{
	GSM_data->enc_hdr_addr = DSP_SPH_HNW_HEAD_UL;
	GSM_data->enc_hb_addr = DSP_DM_ADDR(8, 0x1840);
	GSM_data->enc_mode_addr = DSP_SPH_HNW_COD_MODE_UL;
	GSM_data->dec_hdr_addr = DSP_SPH_HNW_HEAD_DL;
	GSM_data->dec_hb_addr = DSP_DM_ADDR(8, 0x185F);
	GSM_data->dec_mode_addr = DSP_SPH_HNW_COD_MODE_DL;
}
