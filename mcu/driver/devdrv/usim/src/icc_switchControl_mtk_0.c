...
//kal_uint16 L1sim_Cmd(kal_uint8  *txData,kal_uint16  txSize,kal_uint8  expSize, kal_uint8  *result,kal_uint8  *rcvSize, kal_uint8 *Error)
{
   ...
   kal_uint32 rxaddr;
	...
	/* Special Case: AP issued GET DATA with le < Lr (Real response data length), Then expsize should equal  txData[4]*/
	if(5 == txSize && 0xCA == txData[1] && 0 != txData[4])
	{
		expSize = txData[4];
	}
   ...
   if ((SimCard->cmdState != SIMD_CmdIdle)&&(SimCard->cmdState != SIM_StopClk))
   {
    ...
    if (result == NULL)
        DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC15, txSize, *rcvSize, 0, drv_get_current_time(), SimCard->timeout);
    else
        DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC15, txSize, *rcvSize, (kal_uint32) result, drv_get_current_time(), SimCard->timeout);
#endif
...
#if !defined(ATEST_DRV_ENABLE)
        DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC16, hw_cb->simInterface/*SimCard->sim_menu.addr*/, /*SimCard->sim_input.type*/ 0, /*SimCard->sim_input.count*/ 0, SW, *rcvSize);
        if (result == NULL)
            DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC16, txSize, *Error, 0, 0, drv_get_current_time());
        else
   ...
   *isSW6263 = KAL_FALSE;
	...
	for(;;)
	{
...
#if defined(__BYPASS_62XX_63XX__)	
		if(SimCard->bypass6263 && (sw1 == SW1_WARN1 || sw1 == SW1_WARN2))
		{
			kal_sprintf(sim_dbg_str, "return warning status:%x \r\n", status);
			DRV_ICC_print_str(sim_dbg_str); 
		...
		{	// get response  0x61
			...
			{
			    kal_sprintf(sim_dbg_str, "Rx buffer not enough, don't send GET_RESPONSE, SW2=0x%02X, rx_buff_len=%d, rx_len=%d", sw2, rx_buf_len, rx_len);
			    DRV_ICC_print_str(sim_dbg_str);
