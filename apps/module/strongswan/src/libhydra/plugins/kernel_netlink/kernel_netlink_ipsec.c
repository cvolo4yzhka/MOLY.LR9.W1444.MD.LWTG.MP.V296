...
struct private_kernel_netlink_ipsec_t {
   ...
   netlink_socket_t *socket_xfrm;
    ...
    /**
    * Netlink xfrm socket wrapper (IPsec)
    */
    netlink_socket_t *socket_wrapper_xfrm;
#endif
...
#ifdef STRONGSWAN_UNIT_TEST
   /**
    * Netlink xfrm socket wrapper to receive acquire and expire events
   */
    int socket_wrapper_xfrm_events;
#endif
...
static bool handle_events(private_kernel_netlink_ipsec_t *this, struct nlmsghdr *hdr,
      ...
      watcher_event_t event)
{
   ...
   return handle_events(this, hdr, &addr, len);
}
 ...
 */
static bool receive_wrapper_events(private_kernel_netlink_ipsec_t *this, int fd,
      watcher_event_t event)
{
   char response[1024];
   struct nlmsghdr *hdr = (struct nlmsghdr*)response;
   ...
   len = recvfrom(this->socket_wrapper_xfrm_events, response, sizeof(response),
...
#ifdef STRONGSWAN_UNIT_TEST
   if (this->socket_wrapper_xfrm_events > 0)
      lib->watcher->remove(lib->watcher, this->socket_wrapper_xfrm_events);
      close(this->socket_wrapper_xfrm_events);
      policy_entry_destroy(this, policy);

   DESTROY_IF(this->socket_wrapper_xfrm);
#endif
...
#ifdef STRONGSWAN_UNIT_TEST
static status_t register_for_wrapper_events(private_kernel_netlink_ipsec_t *this) {
	struct sockaddr_un wrapper_addr;

	memset(&wrapper_addr, 0, sizeof(wrapper_addr));
	wrapper_addr.sun_family = AF_UNIX;
    strcpy(wrapper_addr.sun_path, "ipsec_event_client.sock");

	/* create and bind RT socket for events (address/interface/route changes) */
	this->socket_wrapper_xfrm_events = socket(wrapper_addr.sun_family, SOCK_DGRAM, 0);
	if (this->socket_wrapper_xfrm_events <= 0)
	{
		DBG1(DBG_KNL, "unable to create XFRM event wrapper socket");
		return FAILED;
	}

	if (bind(this->socket_wrapper_xfrm_events, (struct sockaddr*)&wrapper_addr, sizeof(wrapper_addr)))
	{
		DBG1(DBG_KNL, "unable to bind XFRM event wrapper socket");
		return FAILED;
	}

	lib->watcher->add(lib->watcher, this->socket_wrapper_xfrm_events, WATCHER_READ,
