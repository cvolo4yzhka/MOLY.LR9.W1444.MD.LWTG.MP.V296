...
struct private_ike_sa_t {
	...
	/**
	 * Keep alive via WiFi driver feature supported?
	 */
	bool wdrv_keepalive_feature;
...
METHOD(ike_sa_t, update_wdrv_keepalive_info, void,
	private_ike_sa_t *this, bool enable)
{
	keepalive_info_prop ka_info;

	if (!this->wdrv_keepalive_feature || !this->keepalive)
	{
		return;
	}
	...
	notify_wod(N_WDRV_KEEP_ALIVE, get_name(this) + 4, (void *)&ka_info);
}
	...
	private_ike_sa_t *this)
{
	...
	if (!this->read_cfg)
	{
		this->wdrv_keepalive_feature = this->peer_cfg->use_wdrv_keepalive(this->peer_cfg);
	...
	this->keepalive = TRUE;
	if (this->wdrv_keepalive_feature)
...
METHOD(ike_sa_t, send_mobike_dpd, status_t,
	private_ike_sa_t *this)
{
	DBG1(DBG_IKE, "Send dpd for mobike to refresh NAT");
	this->task_manager->queue_dpd(this->task_manager);
	return this->task_manager->initiate(this->task_manager);
}
	...
	private_ike_sa_t *this, bool address)
{
	...
	if (is_current_path_valid(this))
	{
		...
		else if (address == true && this->state == IKE_CONNECTING)
		{
	...
	if (!is_any_path_valid(this))
	{
		...
		if (!this->oos)
		{
			...
			update_wdrv_keepalive_info(this, FALSE);
						 ...
						 ike_version_t version)
{
	...
	INIT(this,
		.public = {
			...
			.destroy = _destroy,
			.send_dpd = _send_dpd,
			.send_mobike_dpd = _send_mobike_dpd,
			.update_wdrv_keepalive_info = _update_wdrv_keepalive_info,
