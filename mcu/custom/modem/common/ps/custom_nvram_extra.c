...
#if defined(__SUBSIDY_LOCK__) || defined(__CARRIER_RESTRICTION__)
...
static kal_bool smu_gblob_match(
                    ...
                    /* pstrValue */ kal_uint8 *,
                    /* chlen */ kal_uint8);
extern kal_bool sml_query_sml_context_version();
#endif
...
kal_uint8 custom_sim_hardcode_mnc_length(const kal_char *sim_mcc, const kal_char *sim_mnc)
{
    ...
    const kal_char *op_list[] = {/* Indian operators with 3 digit MNC */
                                  "405863", "405840", "405861","405872","405854","405855","405856",
								  "405857", "405858", "405859","405860","405862","405864","405865",
								  "405866", "405867", "405868","405869","405870","405871","405873",
								  "405874"
                                };
...
kal_bool custom_bypass_pin2_puk2_check(kal_uint16 status_words)
{
 ...
 /*****************************************************************************
 * FUNCTION
 *  custom_sim_block_efad_select
 * DESCRIPTION
 *  This function is used to indicate if ef ad select should be blocked for invalid card
 * PARAMETERS
 *  N/A
 * RETURNS
 *  boolean
...
kal_bool custom_sim_block_efad_select(void)
{
...
/***************************************************************************** 
* FUNCTION
*  custom_sim_ignore_iccid_error
*DESCRIPTION
*  This function is used for specific cards whose EF ICCID is pin protected
*  KAL_TRUE: treat 69 82 for read iccid as success and continue init
*  KAL_FALSE: fall back to SIM class on error
*PARAMETERS
* N/A
*RETURNS
* KAL_TRUE or KAL_FALSE
*****************************************************************************/
kal_bool custom_sim_ignore_iccid_error(void)
{
    return KAL_FALSE;
}
...
/***************************************************************************** 
* FUNCTION
*  custom_sim_ignore_file_descriptor
*DESCRIPTION
*  This function is used for specific cards whose file descriptor for EF DOMAIN is incorrect
*  KAL_TRUE: ignore file descriptor for the EF
*  KAL_FALSE: Treat DF ADF file descriptor as error
*PARAMETERS
* N/A
*RETURNS
* KAL_TRUE or KAL_FALSE
*****************************************************************************/
kal_bool custom_sim_ignore_file_descriptor(kal_uint16 file_id)
{
#ifdef __SIM_IGNORE_INVALID_FD__
    if ((file_id == 0x6F03) || (file_id == 0x6F02))
    {
        return KAL_TRUE;
    }
#endif
...
static kal_uint8 smu_gblob_query_u8(smu_gblob_item_enum eItem)
{
    ...
    switch (eItem)
    {
        ...
        case SML_GBLOB_DEV_MAX_RETRY_CNT_U8:
            return info_ptr->dev_max_retry_cnt;
            break;

        case SML_GBLOB_DEV_REMAIN_RETRY_CNT_U8:
            return info_ptr->dev_remain_retry_cnt;
            break;
...
static void smu_gblob_query_a(smu_gblob_item_enum eItem, kal_uint8 *pstrTo, kal_uint8 chlen)
{
    ...
    switch (eItem)
    {
        case SML_GBLOB_IMEI_1_A:
            kal_mem_cpy(pstrTo, info_ptr->imei_1, SML_GBLOB_IMEI_SIZE);
            break;

        case SML_GBLOB_IMEI_2_A:
            kal_mem_cpy(pstrTo, info_ptr->imei_2, SML_GBLOB_IMEI_SIZE);
            break;
...
static void smu_gblob_assign_u8(smu_gblob_item_enum eItem, kal_uint8 chValue)
{
    ...
    switch (eItem)
    {
        ...
        case SML_GBLOB_DEV_MAX_RETRY_CNT_U8:
            info_ptr->dev_max_retry_cnt = chValue;
            break;

        case SML_GBLOB_DEV_REMAIN_RETRY_CNT_U8:
            info_ptr->dev_remain_retry_cnt = chValue;
            break;
...
static void smu_gblob_assign_a(smu_gblob_item_enum eItem, kal_uint8 *pstrValue, kal_uint8 chlen)
{
    ...
    {
        case SML_GBLOB_IMEI_1_A:
            kal_mem_cpy(info_ptr->imei_1, pstrValue, SML_GBLOB_IMEI_SIZE);
            break;

        case SML_GBLOB_IMEI_2_A:
            kal_mem_cpy(info_ptr->imei_2, pstrValue, SML_GBLOB_IMEI_SIZE);
            break;
...
static kal_bool smu_gblob_match(smu_gblob_item_enum eItem, kal_uint8 *pstrValue, kal_uint8 chlen)
{
    ...
    {
        case SML_GBLOB_IMEI_1_A:
            if(kal_mem_cmp(info_ptr->imei_1, pstrValue, SML_GBLOB_IMEI_SIZE) == 0)
            {
                return KAL_TRUE;
            }
            else
        ...
        case SML_GBLOB_IMEI_2_A:
            if(kal_mem_cmp(info_ptr->imei_2, pstrValue, SML_GBLOB_IMEI_SIZE) == 0)
            {
                return KAL_TRUE;
            }
            else
 ...
 *  SMLOBJ
 *******************************************************************************/ 
#if defined (__SMB_SIMME_LOCK__) || (__CARRIER_RESTRICTION__)
...
#endif
{
    ...
    switch(file_idx)
    {
        ...
        case NVRAM_EF_SUBSIDY_LOCK_LID:
            ...
            return sizeof(sml_sl_blob_context_struct);
        ...
        case NVRAM_EF_SML_GBLOB_LID:
            ...
            return sizeof(sml_gblob_s_context_struct);
        
            break;

        case NVRAM_EF_SML_S_LID:
        
            return sizeof(sml_context_s_struct);
...
void sml_init_sml_obj(kal_uint8 source)
{
	...
	if (sbp_query_md_feature(SBP_ENABLE_SUBSIDY_LOCK) == KAL_TRUE)
	{
...
#ifdef __SMB_SIMME_LOCK__
    if (sml_query_sml_context_version() == 1)
    {
                         ...
                         kal_uint8 source)
{
    ...
    switch(item)
    {
        ...
        case SML_CAT_KEY:
            ...
            //ASSERT(*plength == sizeof(SML_CTRL_KEY_STRUCT));
                      ...
                      kal_uint8 source)
{
	...
	if (blob_iter_count != 0)	/* blob has valid key data */
	{
...
#ifdef UNIT_TEST
        {
        	...
        	smu_nvram_write(NVRAM_EF_SUBSIDY_LOCK_LID, SML_SL_NVRAM_ACCESS_ID_UPDATE_LOCK_OP);
