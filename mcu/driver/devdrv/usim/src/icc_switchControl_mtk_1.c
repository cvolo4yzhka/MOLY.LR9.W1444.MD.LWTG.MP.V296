	...
	*ResultVolt = usim_dcb->power;
	...
	//if(usim_dcb->reset_mode == USIM_RESET_NEGOTIABLE)
	{
		if(usim_process_PTS(hw_cb) == KAL_FALSE)
		{
			...
			{
				dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "[SIM_DRV:%d] retry PTS %d\n\r", hw_cb->simInterface, retry);
				if(usim_select_power(usim_dcb->power, hw_cb) == KAL_FALSE) continue;
...
sim_status L1sim_Cmd_MTK(kal_uint8  *txData,kal_uint32  *txSize,kal_uint8  *rxData, kal_uint32  *rxSize, sim_HW_cb *hw_cb)
{
	...
	{
             kal_uint8 *rxDataTMP;
		kal_uint32 rx_buf_len = *rxSize;
             
		if (usimCard->cmd_case == usim_case_4 && txData[1] == 0xA4) {
			rxDataTMP = (kal_uint8*) get_ctrl_buffer(512);
			// SELECT CMD with response data, use temp buffer. For working around VzW UICC case 2.20
			if (rx_buf_len == 0) {
				rx_buf_len = 256;
			}

			*rxSize = 512;
			kal_sprintf(sim_dbg_str, "[%s] Reset rxSize to 512, rx_buf_len:%d", __func__, rx_buf_len);
			DRV_ICC_print_str(sim_dbg_str);
			result = L1sim_Cmd_Layer_MTK(txData, txSize,  rxDataTMP, rxSize, hw_cb, &usimCard->isSW6263);
			
			if (*rxSize > rx_buf_len) {
				*rxSize = rx_buf_len;
			}
			kal_mem_cpy(rxData, rxDataTMP, *rxSize);
                    free_ctrl_buffer(rxDataTMP);
		} else {
